...
def callback(msg):
    ...
    #############################################################################################
         ...
         #   print(3)
              ...
              #print'edge2_dist: %f          ' %(edge2_dist*100), 'edge1_dist: %f          ' %(edge1_dist*100)
              #print'Angle_of_edge2: %f    ' %degrees(j*msg.angle_increment), 'Angle_of_edge1: %f    ' %degrees(i*msg.angle_increment)
              print'lenght of a side is %f      '%length, 'width of a side is %f      '%width
         #print(i)
         #print(j)
         
         temp_ranges = [0] * full
         temp_intensities = [0] * full
         for a in range(full):
             temp_ranges[a] = full_ranges[a]
             temp_intensities[a] = full_intensities
         if (j < i):
            for b in range(j,i+1,1):
                temp_ranges[b] = float("inf")
                temp_intensities[b] = int("0")
         elif (j > i):
            for b in range(j,i+1,1):
                temp_ranges[b] = float("inf")
                temp_intensities[b] = int("0")
         ...
         full_intensities = temp_intensities
         for c in range(360,full):
             full_ranges[c] = float("inf")
             full_intensities = int("0")
